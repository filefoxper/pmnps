...
function fetchPlatforms(){
    ...
    return list.filter((n)=>fs.existsSync(path.join(platsPath,n,'pmnp.plat.json')));
}
...
async function buildAction({plat:startPlat}:{plat?:string}) {
    ...
    info(platform?`start building platform: ${platform}`:`start building platforms`);
    if(!platform){
    ...
    info(`==================== ${platform||''} ====================`);
    const subprocess = execa("npm", ['run','build'],{cwd:path.join(platsPath,platform||'')});
    // @ts-ignore
